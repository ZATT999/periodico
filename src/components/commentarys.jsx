...
import { createCommentary } from "../services/newsService"
...
export function Commentarys({ InitialCommentarys, id }) {
  ...
  const handleDelete = (id) => {
    setCommentarys((prevCommentarys) =>
      prevCommentarys.filter((commentary) => commentary.id !== id)
    )
  }
  ...
  return (
    <section className="flex flex-col w-full h-auto border-t-1 border-gray-300">
      <form onSubmit={handleSumit} className="flex gap-1">
        <textarea
        ...
        >
          <div className="flex justify-between">
            <div className="flex items-center">
              ...
              />
              <div>
                <p className="text-sm font-medium">{commentary.author.name}</p>
              </div>
            </div>
